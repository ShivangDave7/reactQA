...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      ...
      'email':'Joel.Zemlak81@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
    let user = await userDb.updateOne( { 'username':'Karl.Bauch9' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      ...
      'isDeleted':false,
      'username':'Nickolas80',
      'email':'Zena.Simonis18@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
    let admin = await userDb.updateOne( { 'username':'Nickolas80' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
